...
from lib.referformer import build as build_model
...
def get_dataset(image_set, transform,transform_evf, args):
    from data.dataset_refer_bert import ReferDataset
    from data.refsegrs_refer_bert import ReferDataset as rsdataset
    if args.dataset == 'rrsisd':
        ds = ReferDataset(args,
                          split=image_set,
                          image_transforms=transform,
                          image_transforms_evf=transform_evf,
...
def criterion(input, target,text, weight=0.1):
    return Loss(weight=weight)(input, target,text)
...
def evaluate(model, data_loader, epoch):
    ...
    with torch.no_grad():
        for data in metric_logger.log_every(data_loader, 100, header):
            ...
            img, img_evf, target, cap = data
            img, img_evf, target = img.cuda(non_blocking=True), \
                img_evf.cuda(non_blocking=True), \
                target.cuda(non_blocking=True)
            img_device = img.device
            # cap = cap.to(img_device)
            ...
            output,text = model(img, img_evf, target, cap)
            iou, I, U = IoU(output, target)
            loss = criterion(output, target,text)
...
def train_one_epoch(model, criterion, optimizer, data_loader, lr_scheduler, epoch, print_freq,
    ...
    for i, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        del img, img_evf, target, cap, loss, output, data
        # if bert_model is not None:
...
def main(args):
    ...
    param_dicts = [
        {
            "params":
                [p for n, p in single_model.named_parameters()
    ...
    for epoch in range(0, args.epochs):
        ...
        if best:
            ...
            if args.output_dir:
                ...
                model_paths = [os.path.join(output_dir, 'ckpt_model_best')]
                # if (epoch + 1) % 1 == 0:
                #     checkpoint_paths.append(output_dir / f'checkpoint{epoch:04}.pth')
                #     model_paths.append(os.path.join(output_dir,f'ckpt_model{epoch:04}'))
                # for i in range(2):
                for i in range(1):
